...
public sealed class SuperMatterEmitterSystem : SharedEmitterSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<SuperMatterEmitterComponent, ComponentInit>(OnComponentInit);
        SubscribeLocalEvent<SuperMatterEmitterComponent, SuperMatterEmitterValueMessage>(OnApplyMessage);
        SubscribeLocalEvent<SuperMatterEmitterComponent, SuperMatterEmitterEmitterActivateMessage>(OnEmitterActivateMessage);
        SubscribeLocalEvent<SuperMatterEmitterComponent, PowerConsumerReceivedChanged>(ReceivedChanged);
        SubscribeLocalEvent<SuperMatterEmitterComponent, PowerChangedEvent>(OnApcChanged);
        SubscribeLocalEvent<SuperMatterEmitterComponent, GetVerbsEvent<Verb>>(OnGetVerb);
        SubscribeLocalEvent<SuperMatterEmitterComponent, ExaminedEvent>(OnExamined);
        SubscribeLocalEvent<SuperMatterEmitterComponent, AnchorStateChangedEvent>(OnAnchorStateChanged);
        SubscribeLocalEvent<SuperMatterEmitterComponent, SignalReceivedEvent>(OnSignalReceived);
    }
    private void OnSync(Entity<SuperMatterEmitterBoltComponent> entity, ref SyncSuperMatterBoltStats _)
    {
        if (!TryComp<ProjectileComponent>(entity, out var projectileComponent))
            return;
        ...
        if (!TryComp<SuperMatterEmitterComponent>(shootAuthorUid, out var superMatterEmitter))
            return;
        var consumableComponent = EnsureComp<SuperMatterExtraConsumableComponent>(entity);

        consumableComponent.AdditionalEnergyOnConsumption =
            SuperMatterEmitterConsts.GetEnergyFromPower(superMatterEmitter.EnergyToMatterRatio / 100f * superMatterEmitter.PowerConsumption);
        consumableComponent.AdditionalMatterOnConsumption =
            SuperMatterEmitterConsts.GetMatterFromPower((100 - superMatterEmitter.EnergyToMatterRatio) / 100f * superMatterEmitter.PowerConsumption);
    }

    private void OnComponentInit(Entity<SuperMatterEmitterComponent> entity, ref ComponentInit args)
    {
        entity.Comp.PowerUseActive = entity.Comp.PowerConsumption;
        var boltProto = _prototype.Index<EntityPrototype>(entity.Comp.BoltType);
    ...
    private void OnApplyMessage(Entity<SuperMatterEmitterComponent> entity, ref SuperMatterEmitterValueMessage args)
    {
    ...
    public void UpdateBUI(Entity<SuperMatterEmitterComponent> entity)
    {
        var state = new SuperMatterEmitterUpdate(entity.Comp.PowerConsumption, entity.Comp.EnergyToMatterRatio);

        _userInterface.SetUiState(entity.Owner, SuperMatterEmitterUiKey.Key, state);
    }

    private void OnEmitterActivateMessage(Entity<SuperMatterEmitterComponent> entity, ref SuperMatterEmitterEmitterActivateMessage args)
    {
        var emitterComponent = entity.Comp;

        var users = _userInterface.GetActors(entity.Owner, SuperMatterEmitterUiKey.Key);
        Debug.Assert(users.Count() == 1);

        TryActivate(entity, users.First());
        Dirty<SuperMatterEmitterComponent>((entity, emitterComponent));
    }

    private void OnAnchorStateChanged(EntityUid uid, SuperMatterEmitterComponent component, ref AnchorStateChangedEvent args)
    {
        if (args.Anchored)
            return;
    ...
    public bool TryActivate(Entity<SuperMatterEmitterComponent> entity, EntityUid user)
    {
    ...
    private void OnGetVerb(EntityUid uid, SuperMatterEmitterComponent component, GetVerbsEvent<Verb> args)
    {
    ...
    private void OnExamined(EntityUid uid, SuperMatterEmitterComponent component, ExaminedEvent args)
    {
        if (component.SelectableTypes.Count < 2)
            return;
    ...
    private void ReceivedChanged(
        EntityUid uid,
        SuperMatterEmitterComponent component,
    ...
    private void OnApcChanged(EntityUid uid, SuperMatterEmitterComponent component, ref PowerChangedEvent args)
    {
        if (!component.IsOn)
        {
            return;
    ...
    public void SwitchOff(EntityUid uid, SuperMatterEmitterComponent component)
    {
    ...
    public void SwitchOn(EntityUid uid, SuperMatterEmitterComponent component)
    {
        component.IsOn = true;
        if (TryComp<PowerConsumerComponent>(uid, out var powerConsumer))
    ...
    public void PowerOff(EntityUid uid, SuperMatterEmitterComponent component)
    {
    ...
    public void PowerOn(EntityUid uid, SuperMatterEmitterComponent component)
    {
        if (component.IsPowered)
        {
            return;
    ...
    private void ShotTimerCallback(EntityUid uid, SuperMatterEmitterComponent component)
    {
    ...
    private void Fire(EntityUid uid, SuperMatterEmitterComponent component)
    {
    ...
    private void UpdateAppearance(EntityUid uid, SuperMatterEmitterComponent component)
    {
    ...
    private void OnSignalReceived(EntityUid uid, SuperMatterEmitterComponent component, ref SignalReceivedEvent args)
    {
