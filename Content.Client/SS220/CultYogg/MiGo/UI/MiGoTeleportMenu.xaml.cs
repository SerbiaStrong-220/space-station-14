...
using static Content.Client.SS220.CultYogg.MiGo.UI.MiGoTeleportMenu;
...
public sealed partial class MiGoTeleportMenu : FancyWindow
{
    ...
    private readonly Dictionary<string, (ListContainerButton Button, MiGoTeleportTarget target)> _listItems = [];
    ...
    public void Populate(IEnumerable<GhostWarp> warps)
    {
        ...
        var listData = new List<MiGoTeleportListData>();

        foreach (var value in _cultists)
        {
            listData.Add(new MiGoTeleportListData(value.Item2)
            {
                ItemText = value.Item1,
            });
        }

        MiGoTeleportContents.PopulateList(listData);
    }

    private void GenerateButton(ListData data, ListContainerButton button)
    {
        if (data is not MiGoTeleportListData { Target: var netEnt, ItemText: var text })
            return;

        var item = new MiGoTeleportTarget(netEnt, text);
        _listItems[text] = (button, item);
        button.AddChild(item);
        button.AddStyleClass("ButtonSquare");
    }

    public record MiGoTeleportListData(NetEntity? Target) : ListData
    {
