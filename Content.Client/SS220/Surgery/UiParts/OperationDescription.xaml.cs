...
public sealed partial class OperationDescription : Control
{
    ...
    public void UpdateNodeShowTarget(EntityUid patient)
    {
        ...
        foreach (var child in OperationContainer.Children)
        {
            ...
            label.CurrentNodeIcon.ModulateSelfOverride = pair?.Value == label.NodeId ? OperationDescriptionBullet : null;
        }
    }
    ...
    private void AddOperationSteps(SurgeryGraphPrototype graph)
    {
        ...
        {
            var nodeLabel = new OperationNodeLabel(node.Name);
            nodeLabel.NodeRichTextLabel.SetMarkup(Loc.GetString(_nodeNameText, ("nodeName", Loc.GetString(node.Name))));
            AddNodeInfo(node, nodeLabel.NodeRichTextLabel);
