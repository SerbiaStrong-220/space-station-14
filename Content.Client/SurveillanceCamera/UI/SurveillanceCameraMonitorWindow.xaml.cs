...
public sealed partial class SurveillanceCameraMonitorWindow : DefaultWindow
{
    ...
    public SurveillanceCameraMonitorWindow(EntityUid? mapUid)
    {
        ...
        Tabs.SetTabTitle(0, Loc.GetString("surveillance-camera-monitor-ui-list-tab"));
        Tabs.SetTabTitle(1, Loc.GetString("surveillance-camera-monitor-ui-map-tab"));
    }
    ...
    private void AddCameraToMap(string name, string address)
    {
        var item = SubnetList.AddItem($"{name}: {address}");
        item.Metadata = address;
    }
