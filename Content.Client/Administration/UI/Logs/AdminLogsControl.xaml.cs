...
public sealed partial class AdminLogsControl : Control
{
    ...
    public AdminLogsControl()
    {
        ...
        DebugTools.Assert(_clipboard is not null);
        _popup = IoCManager.Resolve<EntityManager>().System<PopupSystem>();
        DebugTools.Assert(_popup is not null);
        _gameTiming = IoCManager.Resolve<IGameTiming>();
        DebugTools.Assert(_gameTiming is not null);
        ...
        ClearSelection.OnButtonDown += (_) =>
        {
            if (_buttonSafetyFreeEnd is null)
            {
                _buttonSafetyFreeEnd = _gameTiming.CurTime.Add(_buttonSafetyFreeDuration);
                ClearSelection.ModulateSelfOverride = Color.Red;
                ClearSelection.Text = Loc.GetString("admin-logs-clear-selection-confirm");
                return;
            }
