ent-ActionCultYoggCorruptItem = Преобразовать предмет через прикосновение
    .desc = Исказить предмет даровав ему форму заложенную создателем.
ent-ActionCultYoggCorruptItemInHand = Преобразовать предмет в руках
    .desc = Исказить предмет у вас в руках даровав ему форму заложенную создателем.
ent-ActionCultYoggPukeShroom = Извергнуть
    .desc = Извергнуть выращенный в желудке гриб.
ent-ActionCultYoggDigest = Микоз
    .desc = Отдать часть калорий, чтобы переварить новый гриб.
ent-ActionCultYoggAscending = Возвышение
    .desc = Проявить свою истинную сущность.
ent-ActionMiGoEnslavement = Порабощение
    .desc = Ввергни плоть в реки ваших мыслей и чувств.
ent-ActionMiGoHeal = Исцеление
    .desc = Ускорьте процессы восстановления прислужника.
ent-ActionMiGoAstral = Выход в астрал
    .desc = Войдите в междумирье.
ent-ActionMiGoErect = Воздвигнуть
    .desc = Воплотите основание великих алтарей.
ent-ActionMiGoSacrifice = Призыв
    .desc = Исполните обряд над закрепленной на алтаре жертвой.
ent-ActionMiGoToggleLight = Внутренний свет
    .desc = Пробудите биолюминесценцию, доступную лишь разуму Ми-Го.ent-ActionMiGoTeleport = Темное наблюдение -- подумать над названием
    .desc = Позволяет просмотреть всех членов культа, а так же прыгнуть к кому-то из астрала.