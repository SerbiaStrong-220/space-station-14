...
public sealed class UseDelaySystem : EntitySystem
{
    ...
    private void OnMapInit(Entity<UseDelayComponent> ent, ref MapInitEvent args)
    {
        ...
        if (!ent.Comp.Delays.ContainsKey(DefaultId)) // SS220 balanced staminaweapons - Only set default delay if it's not already present
            SetLength((ent, ent.Comp), ent.Comp.Delay, DefaultId); // SS220 balanced staminaweapons
    }
