...
public sealed class IntegratedClothingSystem : EntitySystem
{
    ...
    private void ToggleClothing(EntityUid user, Entity<IntegratedClothingComponent> ent)
    {
        ...
        if (_inventorySystem.TryGetSlotEntity(parent, ent.Comp.Slot, out var existing))
        {
            _popupSystem.PopupClient(Loc.GetString(MustRemoveClothingFirst, ("entity", existing)),
                user, user);
            return;
        }
